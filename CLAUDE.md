...
### 다이어그램
- 구조도(아키텍처): SVG 파일로 생성 (**라이트 테마, 흰색 배경**, Pretendard 폰트)
- 흐름도(플로우): mermaid 사용
- 다이어그램은 7개 노드 이하로. 복잡하면 쪼개기.
- 표와 다이어그램이 같은 내용이면 하나만 남기기.
- em dash 금지(`—`). 다이어그램 라벨에도 콜론으로 대체.
- 자세한 톤/SVG 규칙은 `.claude/rules/tone-harness.md` 참조.
...
`error`가 0개여야 한다. 룰 정의는 `scripts/tone-guard.mjs` + `.claude/rules/tone-harness.md` 7.5절.
