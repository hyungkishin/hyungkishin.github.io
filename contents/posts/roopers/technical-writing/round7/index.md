...
나름대로 기준을 세워보긴 했는데, 아직 경험이 부족해서 맞는지는 잘 모르겠어요.
...
API 서버가 여러 대일 때 같은 이벤트를 중복 전송하는 걸 막기 위해서인데요.
...
CDC(Debezium) 같은 binlog 기반 방식을 쓰면 지연을 줄일 수 있지만, 커넥터 운영 부담이 현재 규모에 비해 과하다고 느꼈어요.
...
"at-least-once delivery + idempotent processing"이 현실적으로 할 수 있는 최선이었어요.
...
DB INSERT는 Redis SETNX보다 30배 정도 느리지만(~30ms vs ~1ms), 데이터가 꼬이는 것보다는 낫다고 판단했어요.
...
다만 additive 연산에서도 조심해야 할 게 있었는데요.
...
그래서 metrics 집계에서는 version 체크를 적용하지 않고, 중복 방지는 `event_handled` PK 기반 `tryMarkHandled()`로만 처리했어요.
...
중요한 건 poison pill(파싱 불가능한 메시지)과 이미 처리된 이벤트에서도 반드시 ack를 호출해야 한다는 점이에요.
...
`MAX_POLLING_SIZE`는 1500으로 잡았는데요, 건당 처리 시간이 idempotency INSERT(~30ms) + upsertMetrics(~30ms)로 약 60ms이고, `MAX_POLL_INTERVAL_MS`가 120초이기 때문에 120s / 60ms = 2000건이 이론적 한계예요.
...
3번 재시도해도 실패하면 해당 메시지를 DLT(Dead Letter Topic)로 보내고, offset을 전진시켜서 다음 메시지 처리를 계속해요.
...
이 글에서 정리한 구조가 정답인지는 아직 잘 모르겠습니다.

event_handled 테이블 정리 배치도 아직 안 만들었고, DLT 모니터링도 빠져 있고, hot spot 문제도 트래픽이 진짜 몰려봐야 알 수 있어요.

다만 각 판단 지점에서 "왜 이걸 골랐고 뭘 잃었는지"는 남겨두려고 했습니다.
나중에 이 코드를 다시 열었을 때, 최소한 "그때 왜 이렇게 했더라?"에는 답할 수 있도록.
