...
이 기준이 모든 상황에 맞는 건 아니겠지만, 실제로 좋아요 집계 Consumer가 죽어도 API의 상품 조회가 정상 동작하는 걸 확인했기 때문에, 지금 구조에서는 이 기준이 맞았다고 판단했어요.
...
다만 k6로 500명 동시 요청을 돌려서 정확히 100장만 발급되는 걸 확인했고, couponId partition key에서 hot spot이 생기는 것도 부하 테스트에서 확인한 뒤 "현재 규모에서는 수용 가능하다"는 판단을 내렸어요.
