...
```mermaid
flowchart TD
    A["👥 1만 명 동시 요청"] --> B["🔒 같은 coupon row에 UPDATE 경합"]
    B --> C["⏳ row lock 대기 누적"]
    C --> D["💀 DB 커넥션 풀 고갈"]
    D --> E["🔥 상품 조회, 주문 등 전체 API 마비"]
...
사용자는 발급 요청 후 별도로 결과를 polling 해야 합니다.
...
```mermaid
flowchart LR
    A["📝 DB INSERT 성공"] --> B["📤 Kafka send"]
    B -->|"네트워크 에러"| C["❌ 메시지 유실"]
    C --> D["😱 PENDING 상태 영구 방치"]
...
```mermaid
flowchart LR
    A["📝 DB 트랜잭션\ncoupon_issue_request +\noutbox_event INSERT"] --> B["✅ 커밋"]
    B --> C["⏰ OutboxRelayScheduler\n1초 간격 폴링"]
    C --> D["📤 Kafka send"]
    D -->|성공| E["✅ PUBLISHED"]
    D -->|실패| F["🔄 다음 주기 재시도"]
    ...
    style F fill:#b8860b,color:#ffffff
```

이 구조에서 중요했던 판단이 하나 있는데요.
...
flowchart TD
    subgraph API["commerce-api"]
        A["POST /coupons/issue"] --> B["coupon_issue_request\n+ outbox_event INSERT\n(같은 트랜잭션)"]
        B --> C["✅ 202 Accepted\n+ requestId 반환"]
        D["⏰ OutboxRelayScheduler"] --> E["outbox_event 폴링\nFOR UPDATE SKIP LOCKED"]
        E --> F["📤 Kafka send\n(Avro + Schema Registry)"]
    end

    subgraph KAFKA["Kafka"]
        F --> G["coupon-issue-requests\n(partition key = couponId)"]
    end

    subgraph STREAMER["commerce-streamer"]
        G --> H["CouponIssueConsumer\nmanual ack"]
        H --> I["tryMarkHandled\n(INSERT-first 멱등성)"]
        I -->|"신규"| J["IssueCouponFromQueueUseCase\n수량 확인 + 차감 + 발급"]
        I -->|"중복"| K["skip + ack"]
        J --> L["✅ ack"]
    end

    subgraph POLL["사용자 결과 조회"]
        M["GET /coupons/issue/{requestId}"] --> N["coupon_issue_request\nstatus 확인"]
    end
    ...
    style K fill:#4a4a4a,color:#e0e0e0
    style L fill:#2d6a4f,color:#ffffff
    style M fill:#1a1a2e,color:#e0e0e0
    style N fill:#16213e,color:#e0e0e0
```

Producer 쪽에서는 `acks=all`과 `enable.idempotence=true`로 메시지가 Kafka에 확실히 기록되도록 했고, Avro + Schema Registry로 직렬화를 구조화했습니다.

Consumer 쪽에서는 manual ack를 사용해서, 비즈니스 로직이 완전히 끝난 뒤에만 offset을 커밋하도록 했습니다.
`acknowledge()`를 호출하지 않으면 offset이 올라가지 않기 때문에, Consumer가 중간에 죽어도 해당 메시지는 재전달됩니다.

재전달이 올 수 있으니 `tryMarkHandled()`로 멱등성을 보장하고, 파싱 불가능한 메시지(poison pill)는 로그를 남기고 ack 해서 무한 재전달을 막았습니다.
...
flowchart LR
    ...
    style B fill:#16213e,color:#e0e0e0
    style C fill:#2d6a4f,color:#ffffff
    style D fill:#b8860b,color:#ffffff
    style E fill:#e94560,color:#ffffff
