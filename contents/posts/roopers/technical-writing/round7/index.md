...
문제는 후속 로직이 하나 추가될 때마다 `CreateOrderUseCase`가 뚱뚱해진다는 점이었습니다.
...
"쿠폰 차감이 실패했는데 주문은 성공한 상태"가 생길 수 있습니다.
...
`commerce-api`에서 발생한 이벤트를 `commerce-streamer`라는 별도 애플리케이션이 처리해야 하기 때문입니다.
...
같은 JVM 안에서 끝나고, 후속 처리가 실패해도 핵심 로직에 영향이 없고, 데이터가 유실되어도 괜찮은 경우라면 ApplicationEvent로 충분하다고 판단했습니다.
...
HikariCP 기본 풀 사이즈는 10개이고, InnoDB의 `innodb_lock_wait_timeout`은 기본 50초입니다.
한 트랜잭션이 row lock을 잡고 있는 동안 나머지 요청은 커넥션을 얻기 위해 대기하는데, 10개 커넥션이 전부 lock 대기에 묶이면 11번째 요청부터는 HikariCP의 `connectionTimeout`(기본 30초)을 기다리다 예외가 터집니다.
1만 건이 동시에 들어오면 10건이 lock을 잡고, 나머지 9,990건은 커넥션 풀 밖에서 대기하다가 30초 만에 전부 실패하는 상황이 됩니다.

`UPDATE coupon SET issued_count = issued_count + 1 WHERE id = ? AND issued_count < max_issue_count` 이 쿼리가 동시에 1만 번 실행되면, 같은 row에 대한 InnoDB row lock 경합이 원인입니다.

커넥션 풀이 비면 쿠폰과 상관없는 상품 조회, 주문 목록 같은 API까지 같이 죽습니다.
문제는 쿠폰인데, 죽는 건 서비스 전체라는 것입니다.
...
API 서버가 여러 대일 때 같은 이벤트를 중복 전송하는 걸 막기 위해서입니다. ShedLock 같은 리더 선출 대신 이 방식을 택한 건 모든 인스턴스가 동시에 릴레이에 참여할 수 있어서 처리량이 더 높기 때문입니다.
...
Consumer 입장에서는 같은 메시지가 두 번 올 수 있다는 뜻입니다.
...
Producer는 at-least-once로 발행하고, Consumer는 idempotent하게 처리해서 결과적으로 한 번만 반영되는 것이지, 시스템 레벨에서 exactly-once가 성립하는 건 아닙니다.
...
관심사가 다른 두 가지를 하나에 넣으면 둘 다 어정쩡해집니다.
...
만약 `delivery.timeout.ms`를 `SEND_TIMEOUT_SECONDS`(10초)보다 길게 잡지 않으면 위험한 시나리오가 생깁니다.
Relay가 `future.get(10초)`에서 타임아웃으로 포기하고 Outbox를 FAILED로 마킹했는데, Producer는 백그라운드에서 계속 재시도하다가 12초째에 전송을 성공시키는 경우입니다.
Kafka에는 메시지가 적재됐지만 Outbox는 FAILED 상태라 다음 주기에 재시도하게 되고, 결국 같은 메시지가 두 번 발행됩니다.
15초로 설정한 건 Relay의 10초 타임아웃보다 충분히 크면서도, Producer가 caller보다 먼저 포기하도록 만들기 위해서입니다.
물론 이중 발행이 완전히 불가능한 건 아니지만, Consumer 쪽 `tryMarkHandled()` 멱등성 레이어에서 흡수되기 때문에 최종 결과에는 영향이 없습니다.
...
DLT에 메시지가 쌓였다는 걸 아무도 모르면 의미가 없기 때문입니다.
현재 구현에서는 Outbox DEAD 전환 시 `log.error`로 남기고 있어서, 로그 기반 알림(Slack Appender 등)이 연결되어 있다면 운영자가 감지할 수 있습니다.
이상적으로는 이런 구조가 필요합니다.
kafka-exporter가 DLT 토픽의 consumer lag을 Prometheus에 노출하고, Grafana에서 lag > 0 조건으로 Slack 알림을 보내는 흐름입니다.
운영자가 알림을 받으면 DLT 메시지를 확인하고, 원인을 파악한 뒤 원본 토픽으로 재발행하거나 Outbox DEAD 이벤트를 PENDING으로 되돌리는 방식으로 복구합니다.
이번에는 Kafka 모니터링 인프라까지는 구성하지 않았지만, 구조 자체는 이렇게 확장할 수 있도록 DLT와 DEAD 상태를 분리해 두었습니다.
...
`coupon_issue_request` 테이블 자체가 이미 큐 역할을 하고 있기 때문입니다.
...
rebalance가 일어나는 동안에는 해당 partition의 처리가 일시적으로 멈추기도 합니다.
이런 문제는 partition 수를 늘리거나 consumer를 scale-out 하면 완화되지만, 근본적으로 "분산 시스템은 공짜가 아니다"는 걸 다시 한 번 느꼈습니다.

여기서 한 가지 더 짚어야 할 게 backpressure입니다.
Kafka가 버퍼 역할을 해준다고 해서 문제가 사라지는 게 아니라, 문제가 보이지 않게 되는 것에 가깝습니다.
Producer의 유입 속도가 Consumer의 처리 속도를 지속적으로 초과하면, Kafka 안에 backlog가 쌓이고 consumer lag이 계속 늘어납니다.
선착순 쿠폰에서 이게 무슨 의미냐면, 사용자가 "발급 요청 접수됨"을 받고 polling 했는데 10초가 지나도 PENDING인 상황이 생길 수 있다는 뜻입니다.

현재 구조에서 이걸 막으려면 consumer lag을 모니터링해서, lag이 임계값을 넘으면 consumer를 scale-out 하거나 API 쪽에서 유입을 throttling 하는 판단이 필요합니다.
Kafka는 버퍼일 뿐이고, 시스템 전체의 throughput 한계는 결국 가장 느린 Consumer가 결정합니다.

---

## 📊 실제로 돌려보니 어땠을까?

구조를 다 만들고 나서, k6로 실제 부하 테스트를 돌려봤습니다.

### 선착순 쿠폰 발급 — 500명 동시 요청, 100장 한정

```javascript
// k6 시나리오: 500 VU가 동시에 1회씩 발급 요청
coupon_async_burst: {
...
500명이 동시에 쿠폰 발급 API를 호출하고, 15초 후 각자 결과를 polling 합니다.

| 항목 | 결과 |
|------|------|
| 발급 요청 202 Accepted | 500건 전부 성공 |
| 실제 발급 (SUCCESS) | 100건 |
| 발급 거절 (FAILED) | 400건 |
| 타임아웃 (PENDING) | 0건 |
| 발급 요청 p95 응답 시간 | 200ms 미만 |
| polling 결과 확인까지 소요 | 평균 2~3초 |

정확히 100장만 발급됐고, 초과 발급은 없었습니다.
`incrementIssuedCount`의 `WHERE issued_count < max_issue_count` 조건이 DB 레벨에서 원자적으로 수량을 제한하고, Consumer가 partition 단위로 순차 처리하기 때문에 경합이 최소화된 결과입니다.

발급 요청 자체의 응답 시간이 200ms 미만인 건, API가 Outbox에 INSERT만 하고 바로 202를 반환하기 때문입니다.
실제 발급은 Consumer가 비동기로 처리하니까, 사용자 입장에서는 "빠르게 접수되고, 결과는 나중에 확인"하는 구조가 제대로 동작하는 걸 확인했습니다.

### 이벤트 파이프라인 — 조회/좋아요/주문 부하

```javascript
// 상품 조회: 10→50 VU ramping (40초)
// 좋아요: 5→20 VU ramping (30초)
// 주문: 5→10 VU ramping (30초)
```

| 항목 | p95 응답 시간 | 임계값 |
|------|-------------|--------|
| 상품 조회 | 300ms 이하 | 500ms |
| 좋아요 | 500ms 이하 | 1000ms |
| 주문 | 1200ms 이하 | 2000ms |

세 시나리오 모두 임계값 내로 통과했고, Outbox relay가 이벤트를 Kafka로 정상 발행하는 것, commerce-streamer가 product_metrics를 upsert하는 것, event_handled 테이블에 멱등 처리 기록이 남는 것까지 확인했습니다.

다만 이건 로컬 환경(단일 Kafka 브로커, 단일 DB)에서의 수치라, 운영 환경의 네트워크 지연이나 브로커 복제 오버헤드가 없는 상태입니다.
실제 운영에서는 이보다 느려질 수 있고, 특히 Kafka 브로커 간 복제(`acks=all`)에 의한 추가 지연이 발생합니다.
