...
> 첫번째로 결정해야 했던 건, PG 응답을 얼마나 기다릴 것인가? 였습니다.  
...
그런데 3초 만에 끊었다고 해서 바로 실패로 확정하기엔 아쉬웠는데요,   
네트워크가 순간적으로 흔들린 거라면, 한 번 더 시도하면 성공할 수도 있기 때문이죠.
...
결제 서킷이 열리면 PG 호출이 차단되는데, **복구 스케줄러가 PG에 상태를 조회하는 것까지 막히면 안 됩니다.**
결제가 안 되는 건 괜찮지만, 이미 접수된 결제의 결과를 확인하지 못하면 PENDING/REQUESTED 상태가 영원히 남게 됩니다.
...
"언제 끊을지, 언제 안 보낼지"는 정해졌습니다.
근데 하나 남았습니다. timeout이 났을 때, 이걸 "실패"로 봐도 되는 걸까요?
...
그래서 fallback이든 예외든, **결과를 모르면 PENDING을 유지**하도록 했습니다.

```mermaid
flowchart TD
    A["PG 호출 실패/timeout"] --> B{결과를 아는가?}

    B -->|"PG가 명시적 거절\nsuccess=false + reason"| C["Payment → PENDING 유지\n로그에 사유 기록"]

    B -->|"timeout/네트워크 예외\n결과 자체를 모름"| D["Payment → PENDING 유지\ntxKey = null"]
...
PENDING을 유지하고, 복구 스케줄러가 orderId로 PG에 직접 물어봅니다.
거래가 있으면 반영하고, 없으면 그때 FAILED.
결과가 확정되기까지 최대 5분이 걸리지만, 유령 결제는 안 생깁니다.

그런데 이거보다 먼저 결정했어야 할 게 있었습니다.
PG 호출하는 동안 DB 커넥션을 잡고 있을 건지.
...
|--------------|-------------------|--------|
| 하나의 트랜잭션 | ⚠️ 최대 3초+ | 10건이면 커넥션풀이 바닥나고 전체 장애 |
...
PG가 느려지면 thread가 묶이고, thread가 묶이면 커넥션이 묶이고, 결제랑 상관없는 API까지 죽습니다.

3초에 끊고, 결제 POST는 재시도 안 하고, 실패가 쌓이면 아예 안 보내고, 결과를 모르면 PENDING으로 놔두고 나중에 확인합니다.

트랜잭션을 쪼개서 PG 호출 동안 커넥션을 안 잡고, 서킷을 결제/조회로 나눠서 결제가 터져도 복구 조회는 살려뒀습니다.

완벽하진 않습니다. 5분간 결과를 모르는 구간이 생기고, PG가 50% 성공시키고 있어도 서킷이 열리면 전부 막힙니다.
근데 PG 하나 때문에 서비스 전체가 죽는 것보다는 낫습니다.
