...
Timeout, Circuit Breaker까지 적용하면 "언제 끊을지, 언제 안 보낼지"는 정해진 셈입니다. 그런데 여기서 풀리지 않는 문제가 하나 남습니다. timeout이 났을 때, 이걸 정말 "실패"로 봐도 되는 걸까요?
...
PENDING 유지 + orderId 기반 복구 조회로, "PG가 실제로 성공한 건을 실패로 덮어쓰는" 문제를 방지합니다.
다만 결과가 확정되기까지 최대 5분이 걸리는 트레이드오프가 있습니다.
