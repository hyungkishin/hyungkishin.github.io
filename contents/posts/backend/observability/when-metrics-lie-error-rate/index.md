...
- 실패를 catch로 삼키면 그 실패는 에러율의 분자(실패 수)에서 빠집니다. 분모(전체)는 그대로예요. 그래서 에러율은 실제보다 낮게, 성공률은 높게 찍힙니다.
...
- 정오 배치 대상: 636건.
- 그중 2건은 발송 전에 실패로 찍혔고, 634건이 "전송 성공"으로 기록됐습니다.
...
숫자 없는 번호를 빈 문자열로 저장하는 사용자는 회원 전체의 약 12%였어요. 그들이 그대로 파이프라인 끝까지 갑니다.
...
- 이 배치의 미도달: 180건으로 대상 636건의 약 28%. (빈 번호 사용자 자체는 회원 전체의 약 12%)
