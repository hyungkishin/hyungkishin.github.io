...
여기서 중요한 게 하나 있습니다.  
consumer가 메시지 하나를 오래 붙잡고 처리 중이면, 그 메시지는 이미 읽힌 뒤라 lag에는 안 잡혀요.  
Kafka 밖의 지연도 lag에는 보이지 않습니다. producer가 늦게 넣는 것, 벤더가 접수한 뒤 늦게 배송하는 것 모두요.
...
둘이 이만큼 벌어졌다는 건, 봉투 계산이 애초에 없는 백로그를 상상했다는 뜻입니다.
...
그날의 진짜 미도달은 앞 편에서 본 삼켜진 실패였어요. Kafka가 아니라 발송 판정이 문제였습니다.
