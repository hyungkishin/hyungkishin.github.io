...
장애처럼 터지지는 않았어요. 다만 P95 응답시간이 조금씩 밀리고 있었습니다.
...
숫자만 보면 작아 보였습니다. 하지만 이 reader는 사용자가 계속 밟는 경로였어요.
...
hit률은 맞았습니다. 하지만 네트워크를 건너는 순간 캐시는 이미 한 번 느려집니다.
...
DB 부하가 평소의 N배가 됩니다. DB 알람도 같이 울려요.
...
- L2도 없으면 분산 lock으로 한 pod만 DB로 가서 채워줌. 나머지는 그 값을 기다림.

이 그림이 보인 건, 시도 1과 2가 둘 다 절반만 맞았다는 걸 인정한 다음이었습니다.
Redis Hash는 매 요청 RTT를 없애지 못했습니다. Caffeine 단독은 배포 순간 DB를 지키지 못했고요.
...
사용자가 자기 닉네임을 바꿨어요. 다른 사용자가 들고 있던 그 사용자의 L1 캐시는 옛 닉네임이에요. 1분 동안 옛 닉네임이 다른 화면에 노출됩니다.
컴플레인이 들어옵니다. *"방금 바꿨는데 왜 안 바뀌나요"*

`ExternalLinkConnectionReader` 도 비슷해요.
사용자가 외부 서비스를 방금 연동했어요. 그 사용자 본인 화면에서는 1분 동안 "미연동" 으로 나옵니다. 사고예요.
