...
> 운영 timeout이 자주 떴습니다. 같은 사용자가 며칠째 누락된다는 보고도 들어왔습니다.
>
...
parallelism을 올리면 외부 API가 막습니다. timeout을 늘리면 윈도우가 터집니다.

여기까지 이틀쯤 썼습니다.
부끄러운 건 이틀 동안 같은 질문만 반복했다는 점이에요.
"어떻게 더 빨리 끝낼까"만 보고 있었어요. "같은 사용자가 왜 다시 빠질까"는 뒤로 밀렸습니다.
...
- 1년치 raw 응답이 3초 안에 들어올 가능성은 현실적으로 없습니다. 일정 비율이 매일 timeout이 됩니다.
- silent skip은 추적을 막아요. 다음 실행에서 같은 사용자가 같은 timeout을 또 맞습니다.
- parallelism은 단건 timeout 횟수만 줄일 뿐이에요. 같은 사람 누락 패턴은 그대로고요.
...
둘은 대응이 다릅니다.
하나는 기다리거나 재시도할 문제입니다. 다른 하나는 데이터를 고쳐야 하는 문제예요.
그래서 알림도 갈랐습니다.
...
- **closed-year stat 경로 강제**: 닫힌 연도는 stat 테이블을 직접 읽으면 1차 적재 부하가 줄어듭니다. 호환성 전수 검증 부담으로 후순위로 미뤘습니다. 그래도 마지막에는 이쪽으로 가야 할 가능성이 커요.
...
"왜 같은 사용자가 며칠째 누락되지?" 라는 질문이 어느 날 떠올랐어요. parallelism으론 그 질문이 안 풀린다는 게 보였어요.
