...
>
> 픽스 자리는 코드만이 아니었습니다.
> 사용자가 보고 따라 누르는 문장도 결함의 일부였습니다.

---

## 핫픽스 후에도 왜 알림이 0으로 안 갔을까?

[본 글](../why2/)에서 핫픽스를 머지했는데도 운영 알림 카운트가 0으로 내려가지 않았습니다.

이때 불편했던 건 "아직 배포가 덜 됐다"와 "우리가 막은 결함이 전부가 아니었다"를 바로 구분할 수 없었다는 점이에요.
머지는 끝났고, 알림은 남았습니다.
운영자는 숫자만 봅니다. 그 숫자가 남아 있으면 장애도 아직 남아 있는 것처럼 보입니다.
...
같은 결함이면 머지 후 시간이 지나면서 0으로 수렴해야 정상이에요.
근데 숫자는 내려가다가 다시 튀었습니다.

여기서부터 판단이 갈렸습니다.
배포 문제로 볼 것인가, 같은 결함의 다른 입구로 볼 것인가.
...
이때부터 "어디서 터졌나"보다 "누가 반복해서 터뜨리나"를 먼저 봐야 했습니다.

---

## 왜 같은 사용자가 알림을 여러 번 만들었을까?

운영 알림 로그를 사용자별로 묶었습니다.
시간순 로그로 볼 때는 산발적인 잔여 알림처럼 보였는데, 사용자 기준으로 묶으니 패턴이 바뀌었습니다.
...
> 알림이 안 수렴한 이유는 결함이 그대로 남아서가 아니었습니다.
> 사용자에게 보여준 문장이 막힌 결함을 다시 호출하고 있었습니다.

---

## 같은 뿌리에서 알림은 어디서 갈라졌을까?

세션 덮어쓰기 이후에는 거절 시점에 따라 알림이 두 갈래로 보입니다.
이 분기를 먼저 본 이유는 사용자에게 즉시 보이는 실패만 다시 클릭을 만들기 때문입니다.
...
동기 5xx 분기가 자가증식 루프의 진입점이었습니다.
사용자가 즉시 화면에서 메시지를 보고, 즉시 다시 클릭합니다.

비동기 거절 분기는 사용자가 즉시 행동을 못 하니, 루프가 약합니다.

---

## 픽스 자리는 코드 하나면 충분했을까?

[본 글](../why2/)의 옵션 A·B·C는 결함 자체를 막는 자리였습니다.
그런데 이 사건은 코드 픽스만으로 끝났다고 말하기 애매했어요.

첫 번째 발현은 막아도, 이미 사용자에게 나간 문장이 다음 발현을 만들고 있었습니다.
그래서 픽스 범위를 불편하게 넓혔습니다. 코드가 아니라 문장까지 봐야 했습니다.
...
"다시 시도하세요"를 그대로 내보내지 않는 쪽을 택한 겁니다.
...
>
> 대신 사용자가 같은 장애를 스스로 다시 밟는 경로를 줄였습니다.

`isRecentRetry`의 시간창(30초)을 잘못 잡으면, 실제로 다시 시도해야 하는 사용자에게도 잘못된 가이드가 나갑니다.
...
근데 사용자 메시지가 결함의 트리거 조건을 다시 만들면, 머지가 끝점이 아닙니다.
실패 메시지가 다음 요청의 입력이 됩니다.
결함 하나가 사용자 수만큼, 재시도 횟수만큼 증폭됩니다.
...
장애 분석에서 "어디서 일어났냐"만 보면 이 루프는 안 잡힙니다.
이번에는 "누구한테 반복됐냐"로 묶어야 보였습니다.

---

## 무엇을 아직 못 정했을까?

- **`isRecentRetry` 임계값**: 30초 / 60초 / 120초 어느 게 맞는지 운영 데이터로 조정 필요. 너무 짧으면 의미 없고, 너무 길면 실제 재시도 케이스를 막아요.
...
배포 상태를 확인하고 코드를 다시 본 뒤에도 남은 설명이 없었습니다.
그때까지는 시스템 안에서만 원인을 찾고 있었어요.

단서는 사용자별 묶음에서 나왔습니다.
같은 사람이 같은 실패를 다시 밟고 있다는 사실이 보인 뒤에야, 메시지를 픽스 대상으로 볼 수 있었습니다.
