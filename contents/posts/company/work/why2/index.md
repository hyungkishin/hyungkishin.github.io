...
>
> 핫픽스는 enum 한 줄입니다.  
> 그런데 글이 진짜 짚고 싶은 건, 같은 종류 누락이 다음 status 추가 때 또 일어난다는 점입니다.

---

## 두 알림이 다른 메시지로 떨어졌어요

같은 시간대에 두 종류 알림이 올라오고 있었습니다.
...
"하나는 외부 데이터 소스 거절, 하나는 우리쪽 sign 단계 5xx" 라고 트래킹 나누려고 했어요.
...
어떤 단계든 두 번 트리거되는 순간 두 번째 호출의 세션이 첫 번째를 Redis에서 덮어쓰게 됩니다.

---

## "인증요청 두 번 클릭"이 전부 아닐까요?
...
## 그런데 두 번 안 누른 사용자도 같은 알림을 만들었어요
...
> **포기한 것**: 사용자가 *진짜로* 답변을 수정한 케이스. status는 인증 단계에 머물러 있고 답변만 바뀝니다.

`hiredLast5Years` 같은 답변이 부수 채널 인증 필요 여부를 결정한다면, 이 가드가 잠재 위험을 만들어요.  
운영 데이터상 사전 설문 재제출이 진짜 수정인 경우는 드물고, 대부분 화면 재호출이나 실수라 합의했습니다.  
공짜는 아니었어요.
...
> **포기한 것**: 이 정책이 자가증식 루프와 결합하면 영구 실패 사용자를 만들 수 있어요. 별도 검토 항목.

---

## 그런데 enum set으로는 누락이 막히지 않습니다
...
근본 답은 `when (exhaustive)` 또는 `sealed class` 입니다.
