...
핵심은 `Top1Module`, `HomeTopDefault`, `HomeTopShortForm` 같은 상단 영역을
...
서버에서 데이터를 가져온다고 무조건 빨라지는 건 아니다.
