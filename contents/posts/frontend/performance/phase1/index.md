...
아래와 같은 구조에 맞게 개편을 해야하는데,,, 문제는 현재 *app 디렉토리에 모든 페이지가 개발되어 있는상태.*  

![pages 는 구식 app 은 신식](img_5.png)
 
마치 판매 기준을 어긴것과 같다고 보면 된다. 샌드위치 속 재료를 기준에 맞게 모두 바꿨지만, 포장지가 비닐이 아니라 종이여야 했던 걸 몰랐던 상황처럼.

![안되지롱~!](img_3.png)

그런줄도 모르고 되게 얄미워했네. 열받게...

천만 다행으로, app / pages 라우터 방식을 **모두 지원** 하니 모두 pages 하위로 옮기는 불상사는 발생하지 않는다. 
