---
title: "번들을 줄였는데 점수가 거의 그대로였어요 (CLS/FCP/TBT)"
date: 2025-05-04
update: 2024-05-04
tags:
  - front-end
...
> **TL;DR**
>
> 1부에서 JS 번들을 줄이고 나면 Lighthouse 점수가 따라 오를 거라고 봤어요.  
> 안 올랐습니다. 거의 그대로.
>
> 측정 결과를 다시 까보니 점수를 깎고 있던 건 *번들 크기*가 아니었어요.  
> *CLS* (광고가 본문을 밀어내는 자리), *FCP* (첫 콘텐츠 도착 시점), *TBT* (메인 스레드 차단 시간) 가 따로 놀고 있었습니다.
>
> Core Web Vitals 각 지표가 *어떤 코드 자리* 와 연결되는지 갈래를 그렸어요.  
> 그 위에서 CLS / FCP / TBT 를 *각각* 손봤습니다. 한 번에 풀리는 약은 없었어요.
>
> 이 글은 클라이언트 layer 최적화의 *1단계* 입니다.  
> LCP는 3부에서 따로, 렌더링 경계 자체는 4부에서, 서버/CDN은 5부에서 다룹니다.

---

## 번들을 줄였는데 왜 점수가 안 움직였을까요?
...
## 중간 배포 🙏
이제까지의 개선을 통해, Core Web Vitals 의 3대 요소를 모두 안정화 시켰다.  
Google Core Web Vitals 의 경고가 약간의 조급함을 불러 일으켜서 인지.

현재까지 개선한 부분에 대해서, 주간미팅 에서 팀원 분들께 공유를 드리기로 하였다.   
그리고 중간배포를 진행하는것으로 마무리 되었다.
...
직접 운영했던 서비스에서 이런 수치는 처음본 까닭에 기분은 좋았으나..

빨간 지표로 보이는 LCP (Large Content's Paint) 와 SI (Speed Index) 가 마음을 어렵게 만들었다.

![LCP 4초 실화...](img_9.png)

뭐 어찌됬든 점진적으로 개선하면 될것같다. <strike> 이제 그만 하고싶다 </strike>     

아 참, 반가운 지표가 하나 더 발견되었는데, 중간 배포 이후 구글 Search Console 에서 확인한 지표 결과는 **개선이 필요한 URL** 은 **빠른 URL 과** 자리를 바꾸었다 🥰. 
...
CLS / FCP / TBT는 클라이언트 *조립 단계의 손해* 였어요.  
다음 자리는 *LCP* — 큰 이미지가 *언제 도착하는가* 의 문제로 넘어갑니다.  
그리고 더 큰 흐름에서, *클라이언트 조립 자체를 줄이는* 결정 (RSC) 과 *origin에 도달하기 전에 막는* 결정 (CloudFront) 이 그 뒤에 남아 있었어요.
