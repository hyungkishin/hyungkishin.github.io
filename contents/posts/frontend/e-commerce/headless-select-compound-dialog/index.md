...
전환된 베이스를 당겨오니 머지 충돌이 열다섯 개 났습니다.
...
지우는 쪽을 골랐습니다.
package.json에서 vite가 빠진 순간 기존 파일들은 빌드조차 되지 않습니다. 남겨도 쓸 수 없는 코드였어요.
...
> **포기한 것**: 디폴트 설정을 통째로 신뢰하는 편함. 룰마다 "왜 이 레벨인가"를 다시 다는 비용을 냈습니다.

## 막고 있다고 믿었던 게이트는 비어 있었습니다

그런데 이 커밋을 코드 리뷰에 올리니 구멍이 셋 나왔습니다.
...
플러그인의 peer 범위가 아직 eslint 9까지라는 경고가 리뷰에서 먼저 나왔는데, 실제로 터진 자리였습니다.
버전을 명시 고정하고, 플러그인이 따라오면 되돌리기로 사유를 주석에 남겼어요.

> **포기한 것**: 디폴트를 통째로 버리는 편함. 버릴 때는 버려지는 게 뭔지 목록으로 확인해야 했습니다.
...
value 설계는 사용처의 요구에서 답이 나왔습니다. 문자열이 아니라 옵션 객체 전체였습니다.
...
> **포기한 것**: 마크업까지 제공하는 친절함. 훅이 화면을 모를수록 다른 생김새를 더 받아낼 수 있다고 봤습니다.
...
동작은 했습니다. 그런데 코드 리뷰에서 이런 지적을 받았어요.
하이라이트는 열려 있을 때만 의미가 있는 값인데, 두 상태를 따로 두면 "닫혔는데 하이라이트가 남은 상태"가 타입상 표현 가능하다고요.
지금은 open과 close가 두 setState를 짝지어 부르는 규율로 정합성이 유지되지만, 나중에 `setIsOpen(false)`만 부르는 코드 경로가 하나 생기면 남아 있던 하이라이트가 다음 열기에서 그대로 렌더된다는 겁니다.
...
열려 있는 동안 items가 줄어들면(refetch로 옵션이 5개에서 3개가 되면) 하이라이트 인덱스가 범위 밖에 남아서, 위아래 키가 전부 무반응이 됐습니다.
...
이동이 경계에서 멈추는 건 그대로 뒀어요. 순환을 넣으면 "더 갈 데가 없다"는 신호가 사라지니까요.
...
> **포기한 것**: setState 두 개를 짝지어 부르는 규율로 정합성을 지키는 원래 구조. 규율은 다음 사람이 어기는 순간 끝나서, 타입에 맡겼습니다.
...
controlled와 uncontrolled를 open prop의 "유무"로 판별하는 부분입니다.
...
"통지를 무시하면 안 닫힌다"는 테스트를 일부러 하나 넣었습니다. 내부 상태를 만들지 않는다는 증거가 필요했거든요.
...
이건 코드 리뷰에서 잡혔습니다. 다이얼로그를 두 개 열면 어떻게 되냐는 지적이었어요.
...
이런 버그는 고쳐도, 회귀 테스트로 고정하지 않으면 다음 리팩터링에서 다시 나타납니다.
...
useSelect 테스트는 prop getter와 상태만 씁니다. 내부의 인덱스 계산이나 스택 배열은 등장하지 않아요.
덕분에 다음에 상태 모델을 또 갈아엎어도, 테스트는 한 줄도 바뀌지 않게 됐습니다.
...
headless와 compound를 나란히 만들고 나니, 서로 다른 패턴이 아니라 같은 결이었어요.
...
이 결정은 누가 쥐는 게 맞는지부터 정하고, 나머지는 그 결정이 흔들리지 않게 만드는 일이었어요.
