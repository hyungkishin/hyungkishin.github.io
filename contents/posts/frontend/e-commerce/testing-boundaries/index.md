...
테스트를 더 쓰기 전에 답해야 할 질문이 여기 있었다. 몇 개를 쓸지가 아니라 어떤 실패를 어느 실행 환경에서 잡을 것인가. 15개 검증 대상을 먼저 적고 각 테스트가 빨간불이 될 이유를 정했다. 구현 뒤에는 코드를 열 군데 망가뜨렸고, 단위 규칙 7개 파일에는 변이 158개를 전수로 걸었다.
...
같은 변이를 다시 넣자 이 테스트가 실패했다. 응답을 직접 만들 필요는 없었다. 처음에는 `_pages` 테스트에서 `app/api/_data`를 가져와 응답을 구성했지만 커밋 훅의 아키텍처 검사가 역방향 의존을 막았다. 검증 대상은 응답 내용이 아니라 요청 수다. 기본 MSW 핸들러는 그대로 두고 이벤트만 관찰하자 테스트용 응답 구현도 사라졌다.
...
수동 실험은 코드의 의미를 알고 고른다. 그 장점 때문에 눈에 익은 분기만 다시 확인하기 쉽다. 단위 규칙이 있는 7개 파일에 Stryker를 걸자 변이 158개 중 11개가 살아남았다.
...
Stryker를 `pnpm check`에는 넣지 않았다. 실행마다 67~73초가 들지만 시간만이 이유는 아니다. 변이 점수는 합격선이 아니다. 살아남은 변이가 실제 구멍인지, 도달할 수 없는 코드인지, 다른 하네스가 맡은 경계인지 읽어야 한다. 상시 게이트에 점수를 걸면 이 판단은 100%를 만드는 작업으로 바뀐다. 단위 로직을 크게 바꿀 때 실행하는 탐색 도구로 남겼다.
...
테스트 피라미드의 층을 개수로 정하면 이 차이가 사라진다. 순수 규칙은 단위 테스트로 내리고 여러 코드의 배선은 통합 테스트에서 본다. 실제 브라우저가 결과를 결정하면 E2E까지 올린다. 변이 테스트는 그 배치가 놓친 자리를 찾지만 모든 층을 대신하지 않는다.

최종 게이트에는 node와 jsdom, Storybook, lint, typecheck, build, E2E가 들어간다. Stryker는 게이트 밖에서 살아남은 변이를 해석한다. 토글이 깨지면 모델 테스트가 실패하고 캐시 재사용이 깨지면 요청을 보는 통합 테스트가 실패한다. 히스토리를 덮어쓰면 304개가 다시 초록이어도 브라우저의 두 테스트가 멈춘다.
