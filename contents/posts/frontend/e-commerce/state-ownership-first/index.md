...
같은 판정이 자동 재시도에도 걸린다. 400대는 다시 보내도 결과가 같으므로 자동 재시도 대상에서 빠진다. 서버 오류와 네트워크 실패와 타임아웃에만 한 번 더 시도한다.
...
여기서 장애가 번지는 범위가 갈린다. 목록 API가 죽으면 결과 영역만 에러가 된다. 필터와 헤더와 장바구니 상태는 산다. 홈은 다르다. 단일 endpoint라 하나가 죽으면 홈 전체가 에러 화면이 된다. 헤더가 layout에 있어 다른 페이지로 나가는 길만 남는다.

응답이 아예 오지 않는 장애는 타임아웃 10초로 끊는다. 자동 재시도가 한 번 붙으므로 사용자가 에러 화면을 보기까지 실제로는 최대 20초가 걸린다. 그래도 무한 로딩보다 낫다고 봤다. 로딩이 끝나지 않는 화면은 재시도할지 떠날지 고를 근거조차 주지 않는다.
...
이 구분이 작동하는 범위는 계약 안의 200 응답까지다. 계약을 벗어난 응답은 렌더 예외가 된다.

> **보류한 것**: 렌더 예외를 격리할 Error Boundary. 안 만든 게 아니라 지금 검증할 수 없어서 미뤘다. mock API는 계약을 벗어난 200을 만들지 못한다. 막을 대상을 재현할 수 없는 상태에서 안전망을 먼저 두면, 동작하는지 모르는 코드가 남는다. 페이지 경계를 어디에 둘지와 함께 정해야 하는 문제이기도 하다.
...
조건에는 URL에 없는 값도 하나 섞인다. pageSize다. 이건 사용자가 바꾸는 값이 아니라서 URL에 남기지 않는다. 그런데 화면이 직접 끼워 넣으면 호출자마다 다른 값을 넣을 수 있고, 그러면 같은 URL이 다른 query key와 다른 요청이 된다. 조립하는 자리를 훅 하나로 고정했다.
...
export const useProductListCondition = () => {
  ...
  return { condition, setFilters, canResetFilters: hasNonDefaultFilters(filters) }
}
```

URL이 확정하는 값과 화면이 붙이는 값이 섞여도 조립 지점이 하나면 결과는 하나다. 두 곳에서 조립하는 순간 다시 갈린다.
