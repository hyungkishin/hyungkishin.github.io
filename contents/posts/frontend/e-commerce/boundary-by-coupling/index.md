---
title: "5주차에 없앤 죽은 버튼이 다른 경로로 되살아나 있었다"
date: 2026-07-31
update: 2026-07-31
tags:
  - front-end
...
구조를 나누는 근거로 개수를 쓰면 매번 임계값을 정해야 한다. 슬라이스 여덟 개 중 몇 개에 `index.ts`를 둘지, 상태 하나에 store를 몇 개 쓸지, status 몇 번대를 경계로 올릴지. 세 번 다 개수로 정했다가 세 번 다 뒤집혔다.
...
| --- | --- |
| A | 통합 모델과 통합 store |
| B | 모델만 분리하고 하나의 store에서 조립 |
| C | 모델과 store 모두 분리 |

B를 골랐다. C를 반려한 근거는 `runtime을 분리해야 할 근거가 없고 reset 조율과 소비 방식의 복잡도만 증가한다`였다.
...
셋 다 원래 결정 하나를 깨거나 더 큰 결합을 만든다. C로 바꿨다.
...
결합으로 정하면 임계값이 필요 없다. 서로를 알아야 하면 같이 둔다. 몰라도 되면 나눈다. 답이 하나로 정해진다.
