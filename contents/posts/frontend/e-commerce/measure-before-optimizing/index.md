...
홈 LCP는 42,175ms였다. Hero 이미지를 7.5MB에서 32KB로 줄이자 3,131ms가 됐다. 그런데 FCP는 오히려 899ms 늦어졌다.

FCP만 되돌리기는 쉬웠다. 서버 셸을 없애고 `Loading home…`을 먼저 보여주면 된다. 그렇게 하지 않았다. 빨리 보이는 로딩 문구와 조금 늦게 보이는 실제 홈은 같은 화면이 아니기 때문이다.
...
이때 “이미지를 줄이자”만으로는 부족했다. 원본을 압축할지, preload를 붙일지, 브라우저가 작은 후보를 고르게 할지에 따라 고치는 곳이 달라진다. 전송량과 요청 시작 시점을 따로 확인했다.
...
LCP는 42,175ms에서 3,131ms로 내려왔다. 여기까지는 예상대로였다.
...
## 평균 하나로는 서버 대기도 설명되지 않았다

이후 상품 목록 metadata가 서버 응답을 기다리게 됐다. 검색어와 카테고리를 공유 카드에 넣으려면 목록 조회가 끝나야 한다. 이 대기가 사용자 TTFB까지 막는지 궁금했다.
...
본문과 metadata가 같은 데이터를 두 번 부르는 문제도 남았다. 요청 범위 안에서 조회 결과만 공유해 Route Handler 호출을 한 번으로 줄였다. 사용자 요청 사이에 캐시를 공유하지는 않았다.

> **포기한 것**: 크롤러의 빠른 TTFB. 응답 내용으로 공유 카드를 만들려면 크롤러는 조회를 기다린다. 대신 일반 사용자의 셸은 그 대기에서 빠져나온다.

## 클릭 128ms 중 핸들러는 1ms였다

마지막 측정은 찜 버튼이었다. 24개 카드 중 하나를 눌렀을 때 상호작용 duration 중앙값은 128ms였다. 정작 이벤트 processing은 1ms였다. presentation delay가 117ms를 차지했다.
...
그래서 측정 전에 바뀌지 않을 것을 정했다. Hero는 같은 이미지를 쓴다. 홈 제목을 로딩 뒤로 숨기지 않는다. 카드 24개와 화면 계산을 유지한다. 찜 버튼은 누르는 즉시 바뀐다.

그 조건에서 홈 LCP는 42,175ms에서 3,131ms로 줄었고 CLS는 0.065에서 0이 됐다. 상품 목록 LCP는 3,217ms에서 2,465ms가 됐다. URL 복원, 뒤로가기와 앞으로가기, 빈 상태, 오류와 재시도도 다시 확인했다.
