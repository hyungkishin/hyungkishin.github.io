...
홈 LCP를 42,175ms에서 3,384ms로 줄이자 FCP가 899ms 늦어졌다. 되돌리지 않았다. 빨리 보이는 `Loading home…`과 조금 늦게 보이는 실제 홈은 같은 화면이 아니다.
...
| 구간 | Before | After |
| --- | ---: | ---: |
| Hero 전송량 | 7,545,525B | 162,058B |
| Hero 요청 시작 | 626ms | 41ms |
| LCP 중앙값 | 42,175ms | 3,384ms |

`next/image`에 실제 렌더 폭을 나타내는 `sizes`를 주자 브라우저가 1920px 후보를 골랐다. Hero는 빌드할 때부터 알고 있는 이미지라 초기 HTML에서 요청 힌트도 보낼 수 있었다. 파일은 약 46분의 1이 됐고 요청은 585ms 일찍 시작됐다.

### 박스 폭을 적자 화질이 깨졌다

첫 측정에서는 모바일 `sizes`에 Hero 박스 폭인 388px을 적었다. LCP는 3,131ms, 전송량은 32,425B까지 줄었다. 하지만 그 값은 올바른 최종 결과가 아니었다.

모바일 Hero 박스는 4:5이고 원본은 16:9다. `object-fit: cover`가 높이를 채우면 이미지가 좌우로 넘친다. 박스 폭을 `W`라고 할 때 실제로 그려지는 이미지 폭은 다음과 같다.

```text
W × 5/4 × 16/9 = W × 20/9
```

412px viewport에서는 862 CSS px, DPR 1.75에서는 1,509 물리 px가 필요했다. 750px 후보는 화면에서 2.01배 확대됐다. 가방의 가죽 결이 뭉개진 이유다.

`sizes`를 cover 기하에 맞추자 후보는 1920px이 됐다. 원본 대비 픽셀의 평균 채널 차이는 3.12에서 1.46으로 줄었다. 대신 전송량은 32KB에서 162KB, LCP는 3,131ms에서 3,384ms로 늘었다.

253ms는 측정 노이즈보다 컸다. 그래도 고친 값을 택했다. 3,131ms는 화질 계약을 깨서 얻은 숫자였기 때문이다. 남은 낭비도 있다. 가로 이미지의 45%만 모바일 박스에 보인다. 이를 줄이려면 모바일용 세로 crop을 별도 에셋으로 제공해야 한다.
...
일반 UA에는 서버 셸이 먼저 왔다. metadata는 조회가 끝난 뒤 스트리밍됐다. `facebookexternalhit` UA는 metadata가 준비될 때까지 첫 byte도 받지 못했다. 이 UA가 Next.js의 metadata streaming 대상에서 제외된 결과다. 크롤러가 부분 HTML을 어떻게 처리하는지는 이 측정으로 확인하지 않았다.
...
## 중복 요청과 타임아웃 중 하나를 버려야 했나

metadata와 본문 prefetch는 같은 상품 목록을 읽는다. document 한 건에서 Route Handler가 몇 번 실행되는지 서버 내부 카운터로 셌다.

| 구성 | Route Handler 호출 |
| --- | ---: |
| 요청 범위 `cache()`와 timeout signal | 1회 |
| `cache()` 제거, signal 유지 | 2회 |
| `cache()` 제거, signal 제거 | 1회 |
| `cache()` 제거, 같은 signal 객체 공유 | 2회 |

Next.js 16.2.10의 이 요청 경로에서는 signal이 포함된 fetch가 native memoization되지 않았다. 같은 signal 객체를 두 호출에 넘겨도 결과는 2회였다.

native memoization으로 1회를 만들려면 `AbortSignal`을 빼야 했다. 그러면 fetch 자체를 10초에 중단하는 수단도 사라진다. 응답 없는 서버 조회는 metadata를 기다리는 UA의 document까지 붙잡는다. 중복 제거 효과가 같은데 요청 중단만 잃을 이유는 없었다. URL 문자열을 키로 쓰는 요청 범위 React `cache()`를 남겼다.

이 결론은 모든 fetch에 대한 규칙이 아니다. Next.js 16.2.10과 이 요청 경로에서 네 구성을 비교한 결과다.
...
그래서 측정 전에 바뀌지 않을 것을 정했다. Hero의 구도와 피사체를 유지한다. 홈 제목을 로딩 뒤로 숨기지 않는다. 공유 카드에는 실제 검색 결과를 담는다. 카드 24개와 화면 계산을 유지한다.
