...
이 조회는 서버가 자기 Route Handler를 HTTP로 부른다. 실무 기준에서는 그렇게 두지 않는다. 홉이 하나 늘고 그 대가로 얻는 것이 없다. 직렬화와 TCP 연결과 미들웨어를 한 번 더 탄다. 위 4ms는 로컬이라 나온 값이고 컨테이너나 서버리스에서는 훨씬 비싸다. `APP_ORIGIN`도 설정 의존이라 build와 runtime 값이 어긋나면 조용히 깨지고, 컨테이너 안에서 외부 origin으로 못 나가는 경우도 있다. 기본값 없이 던지게 둔 이유가 여기 있다. Route Handler는 외부 클라이언트용 공개 계약인데 서버 내부가 그 계약을 다시 소비하면 소유자가 애매해진다.

지금 구현이 그 구조인 이유는 하나다. 요구 명세가 `APP_ORIGIN`을 끊어 metadata 조회 실패를 재현하는 절차를 못박아 뒀다. 공유 함수 직접 호출로 바꾸면 그 재현 자체가 성립하지 않는다.

바꾼다면 서버 전용 조회 함수를 하나 두고 Route Handler와 RSC가 둘 다 그것을 부른다. 요청 범위 `cache()`로 중복을 제거하는 구조는 그대로 남는다. fetch가 사라지면 native fetch memoization도 사라지므로, 중복 제거를 `cache()`에 둔 선택이 그때 값을 한다.
